"""Auto-generate visualization when new checkpoints are saved."""
...
OUTPUT_DIR = '/workspace/xuannv/aef_reference/outputs/aef_distill_v2'
VIZ_DIR = os.path.join(OUTPUT_DIR, 'auto_viz')
os.makedirs(VIZ_DIR, exist_ok=True)

# Load dataset (use first sample for consistency)
...
aef_emb = batch['aef_embedding'][0].to(device)
aef_emb = aef_emb.permute(1, 2, 0)  # H, W, D
...
def to_pca_rgb(emb):
    ...
    flat_emb = emb_np.reshape(-1, D) - mean
    proj = flat_emb @ eigvecs[:, :3]
...
while True:
    ...
    for f in os.listdir(OUTPUT_DIR):
        if f.startswith('step_') and f.endswith('_seed42.pt'):
    ...
    for step, fname in checkpoints:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                student_emb = out['teacher_embeddings'][0]  # H, W, D
            
            stu_rgb = to_pca_rgb(student_emb)
            aef_rgb = to_pca_rgb(aef_emb)
            diff = np.abs(stu_rgb - aef_rgb).mean(axis=-1)
            stu_corr = analyze_stripes(student_emb)
            
            fig, axes = plt.subplots(1, 3, figsize=(15, 4))
            axes[0].imshow(stu_rgb)
            axes[0].set_title(f'Step {step} PCA RGB\nmax_y_corr={stu_corr:.3f}')
            axes[1].imshow(aef_rgb)
            axes[1].set_title('AEF PCA RGB')
            im = axes[2].imshow(diff, cmap='hot')
            axes[2].set_title(f'|Student-AEF|\nmean={diff.mean():.3f}')
            plt.colorbar(im, ax=axes[2], fraction=0.046)
            
            plt.tight_layout()
            save_path = os.path.join(VIZ_DIR, f'viz_step_{step:06d}.png')
    ...
    # Stop if step 2000 is processed
    if 2000 in processed:
        print('All checkpoints processed. Exiting.')
        break
