"""Test encoder without spatial_fusion"""
import sys
...
with torch.no_grad():
    ...
    # No spatial_fusion: bypass it
    # We need to manually run encoder forward without spatial_fusion
    B, T, H, W, C = x.shape
    ...
    # Skip spatial_fusion, just apply norm
    feats_no_sf = encoder.norm(final_features)
    print(f"Encoder (no spatial_fusion) y-corr: {y_corr(feats_no_sf):.4f}")
    
    # Test summarizer with both
    mu_direct = summarizer(feats, ts, valid_periods)
    print(f"Summarizer (direct) y-corr: {y_corr(mu_direct):.4f}")
    
    mu_no_sf = summarizer(feats_no_sf, ts, valid_periods)
    print(f"Summarizer (no spatial_fusion) y-corr: {y_corr(mu_no_sf):.4f}")
    
    # Check PCA
    for name, mu in [("direct", mu_direct), ("no_sf", mu_no_sf)]:
        emb = mu[0].cpu().numpy()  # (128, 128, 64)
        ...
        print(f"  {name} PCA top-3 cumulative: {eigvals[:3].sum() / total_var:.4f}")
