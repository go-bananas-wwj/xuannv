"""Test model with both spatial_fusion and spatial_smooth removed"""
import sys
...
with torch.no_grad():
    ...
    # Summarizer without spatial_smooth
    B, T, H, W, C = feats_no_sf.shape
    # Skip spatial_smooth, directly do time_pool
    q = summarizer.summarizer_q(valid_periods)
    z = summarizer.time_pool(feats_no_sf, q, mask=None)
    print(f"After time_pool (no spatial_smooth) y-corr: {y_corr(z):.4f}")
    
    mu = summarizer.proj_64(z)
    print(f"After proj_64 (no spatial_smooth) y-corr: {y_corr(mu):.4f}")
    ...
    # Also check channel correlation
