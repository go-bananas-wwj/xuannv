"""Debug why two scripts give different y-corr"""
import sys
...
with torch.no_grad():
    ...
    # Compute y-corr BEFORE spatial_fusion
    y_before = y_corr(final_features)
    print(f"Before spatial_fusion: y-corr={y_before:.4f}")
    ...
    y_after_sf = y_corr(final_features)
    print(f"After spatial_fusion: y-corr={y_after_sf:.4f}")
    
    out_manual = encoder.norm(final_features)
    y_after_norm = y_corr(out_manual)
    print(f"After norm: y-corr={y_after_norm:.4f}")
    
    # Direct
    out_direct = encoder(x, ts)
    y_direct = y_corr(out_direct)
    print(f"Direct: y-corr={y_direct:.4f}")
    print(f"Max diff manual vs direct: {(out_manual - out_direct).abs().max().item():.6f}")
    
    # Check: is y_corr sensitive to tiny numerical differences?
    print(f"\nIs y-corr same for manual and direct? {abs(y_after_norm - y_direct) < 1e-6}")
