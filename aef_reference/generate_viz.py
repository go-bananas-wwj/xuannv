"""Generate visualization for a given checkpoint"""
import sys
sys.path.insert(0, "/workspace/xuannv")
sys.path.insert(0, "/workspace/xuannv/aef_reference")

import torch
import numpy as np
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import matplotlib.gridspec as gridspec
import torch_npu
from src.aef.architecture.aef_module import AlphaEarthFoundations
from src.aef.data.haidian_dataset import HaidianAEFDataset, collate_fn

def _embed_to_rgb_shared(emb, basis):
    emb_n = emb / (np.linalg.norm(emb, axis=-1, keepdims=True) + 1e-8)
    proj = emb_n @ basis
    proj = (proj - proj.min()) / (proj.max() - proj.min() + 1e-8)
    return np.clip(proj, 0, 1)

def generate_viz(checkpoint_path, output_dir, patch_ids=["patch_000036"]):
    input_sources = {"s1": 2, "s2": 6, "tianyi_sar": 1, "landsat": 6, "planet": 4}
    decode_sources = {"s1": 2, "s2": 6, "tianyi_sar": 1, "landsat": 6, "planet": 4, "dem": 1, "worldcover": 11, "dynamic_world": 9, "jrc_water": 1}
    model = AlphaEarthFoundations(
        model_size="small",
        input_sources=input_sources,
        decode_sources=decode_sources,
        per_source_latent=32,
        enable_text_align=False,
    )
    
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    state_dict = checkpoint["model_state_dict"]
    new_state_dict = {}
    for k, v in state_dict.items():
        if k.startswith("module."):
            new_state_dict[k[7:]] = v
        else:
            new_state_dict[k] = v
    model.load_state_dict(new_state_dict, strict=False)
    
    device = "npu:0"
    model = model.to(device)
    model.eval()
    
    # Load dataset
    dataset = HaidianAEFDataset(
        data_root="/workspace/xuannv/data_raw/haidian/scenes",
        planet_root="/workspace/xuannv/data_raw/beijing/planetscene",
        source_names=["s1", "s2", "tianyi_sar", "landsat", "planet"],
        aef_embedding_root="/workspace/xuannv/data_raw/haidian/aef_embeddings/haidian_2025_patches",
        split="train",
        start_date="2025-12-01",
        end_date="2026-04-30",
    )
    
    target_patches = set(patch_ids)
    samples_to_viz = []
    for sample in dataset:
        if sample["patch_id"] in target_patches:
            samples_to_viz.append(sample)
            target_patches.discard(sample["patch_id"])
        if len(target_patches) == 0:
            break
    
    if not samples_to_viz:
        print("No target patches found")
        return
    
    import os
    os.makedirs(output_dir, exist_ok=True)
    
    for sample in samples_to_viz:
        patch_id = sample["patch_id"]
        batch = collate_fn([sample])
        
        source_data = {k: v.to(device) for k, v in batch["source_data"].items()}
        timestamps = {k: v.to(device) for k, v in batch["timestamps"].items()}
        valid_periods = batch["valid_periods"]
        
        with torch.no_grad():
            out = model(source_data, timestamps, valid_periods)
        
        student_emb = out["embeddings"][0].detach().cpu().numpy()  # (H, W, 64)
        
        aef_emb_path = f"/workspace/xuannv/data_raw/haidian/aef_embeddings/haidian_2025_patches/{patch_id}.npy"
        aef_emb = np.load(aef_emb_path)  # (64, H, W)
        aef_emb = np.transpose(aef_emb, (1, 2, 0))  # (H, W, 64)
        
        # PCA shared basis
        D = 64
        flat = np.concatenate([student_emb.reshape(-1, D), aef_emb.reshape(-1, D)], axis=0)
        U, S, Vt = np.linalg.svd(flat - flat.mean(axis=0), full_matrices=False)
        basis = Vt[:3].T
        
        student_rgb = _embed_to_rgb_shared(student_emb, basis)
        aef_rgb = _embed_to_rgb_shared(aef_emb, basis)
        diff = np.abs(student_rgb - aef_rgb).mean(axis=-1)
        
        # Plot
        fig = plt.figure(figsize=(16, 5))
        gs = gridspec.GridSpec(1, 3, figure=fig, wspace=0.05)
        
        ax = fig.add_subplot(gs[0, 0])
        ax.imshow(student_rgb)
        ax.set_title("Student (PCA RGB)", fontsize=12, fontweight='bold')
        ax.axis('off')
        
        ax = fig.add_subplot(gs[0, 1])
        ax.imshow(aef_rgb)
        ax.set_title("AEF Official (PCA RGB)", fontsize=12, fontweight='bold')
        ax.axis('off')
        
        ax = fig.add_subplot(gs[0, 2])
        im = ax.imshow(diff, cmap='hot')
        ax.set_title("|Student - AEF|", fontsize=12, fontweight='bold')
        ax.axis('off')
        plt.colorbar(im, ax=ax, fraction=0.046)
        
        plt.suptitle(f"{patch_id} @ Step 200", fontsize=14, fontweight='bold', y=0.98)
        save_path = os.path.join(output_dir, f"viz_step_000200_{patch_id}.png")
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()
        print(f"Saved {save_path}")

if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Usage: python generate_viz.py <checkpoint_path> <output_dir>")
        sys.exit(1)
    generate_viz(sys.argv[1], sys.argv[2])
