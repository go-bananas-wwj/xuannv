...
class LearnedSpatialResampling(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, scale_factor: float):
        ...
        if scale_factor > 1:
            # Upsampling: use resize-conv with anti-aliasing avg_pool + TWO conv layers
            # to increase receptive field and better blend interpolation blocks
            ...
            # Anti-aliasing: small avg pool after upsample to smooth interpolation edges
            self.anti_alias = nn.AvgPool2d(kernel_size=3, stride=1, padding=1)
