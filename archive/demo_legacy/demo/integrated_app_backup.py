...
PCA_RGB_PATH = "/workspace/outputs/aef_qwen_v1/pca_rgb.npy"
GLOBAL_PCA_PATH = "/workspace/outputs/aef_qwen_v1/global_pca.npy"
...
# ──────────────────────────────────────────
def _build_mosaic():
    """Build global mosaic from all patches."""
    bounds_data = []
    ...
    max_x = max(b[1]+W for b in bounds_data)
    max_y = max(b[2]+H for b in bounds_data)

    global_w = int((max_x - min_x))
    global_h = int((max_y - min_y))
    mosaic = np.zeros((global_h, global_w, 3), dtype=np.uint8)

    for pid, bx, by, idx in bounds_data:
        emb = EMB_L2[idx]
        img = emb_to_rgb(emb, use_global=True)
        px = int(bx - min_x)
        py = int(by - min_y)
        mosaic[py:py+H, px:px+W] = np.array(img)
