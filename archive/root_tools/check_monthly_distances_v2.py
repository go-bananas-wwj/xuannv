...
pids = ds.patches[:5]
print("V2 Main Model (epoch_499) - Monthly Embedding Distances")
print(f"Sampled {len(pids)} patches")

# Precompute all embeddings for these 5 patches
embs_month = {}
embs_period = {}
for i, pid in enumerate(pids):
    ...
    for name, w in MONTHS.items():
        try:
            embs_month[pid][name] = get_emb(i, w)
        except Exception as e:
            pass
    embs_period[pid] = {}
    for name, w in PERIODS.items():
        try:
            embs_period[pid][name] = get_emb(i, w)
        except Exception as e:
            pass
    print(f"  Patch {pid} done ({i+1}/{len(pids)})")
