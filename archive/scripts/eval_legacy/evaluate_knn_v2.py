...
def knn_predict_npu(X_train, y_train, X_test, k, device):
    ...
    # 使用 CPU 计算以避免 NPU 内存碎片
    X_train_t = torch.from_numpy(X_train).cpu()
    y_train_t = torch.from_numpy(y_train).long().cpu()
    batch_size = 256
    preds = []
    for i in range(0, len(X_test), batch_size):
        batch = torch.from_numpy(X_test[i:i+batch_size]).cpu()
        dist = torch.cdist(batch, X_train_t)
        _, idx = dist.topk(min(k, len(X_train)), largest=False, dim=1)
        neighbor_labels = y_train_t[idx].numpy()  # [B, k]
