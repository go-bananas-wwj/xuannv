#!/usr/bin/env python3
"""KNN 下游评估 — PyTorch 实现，直接在 NPU 上跑."""
...
def knn_predict_npu(X_train, y_train, X_test, k, device):
    """PyTorch KNN on NPU. X_train: [N, D], y_train: [N], X_test: [M, D].
    ...
    for i in range(0, len(X_test), batch_size):
        ...
        dist = torch.cdist(batch, X_train_t)  # [batch, N_train]
        _, idx = dist.topk(min(k, len(X_train)), largest=False, dim=1)
        neighbor_labels = y_train_t[idx]  # [batch, k]
        # majority vote
        mode_vals = []
        for j in range(neighbor_labels.shape[0]):
            vals, counts = torch.unique(neighbor_labels[j], return_counts=True)
            mode_vals.append(vals[counts.argmax()].item())
        preds.extend(mode_vals)
    return np.array(preds)

def knn_eval(embeddings, labels_dict, k, device, num_classes):
    ...
    all_labels = sorted(set(all_trues) | set(all_preds))
    label_to_idx = {l: i for i, l in enumerate(all_labels)}
    cm = confusion_matrix(all_trues, all_preds, labels=all_labels)
    per_class_iou = {}
    for c in all_labels:
        i = label_to_idx[c]
...
def main():
    ...
    for task_name, label_dir, fname, num_classes in DOWNSTREAM_TASKS:
        ...
        for k in [5, 20]:
            ...
            m = knn_eval(emb_c, lbl_c, k, device, num_classes)
            if m:
