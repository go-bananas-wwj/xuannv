#!/usr/bin/env python3
"""DDP V6 Enhanced Temporal 训练入口 — GPU 6/7 双卡并行.

用法:
    cd /workspace/xuannv
    CUDA_VISIBLE_DEVICES=6,7 torchrun --nproc_per_node=2 \
        scripts/train/train_ddp_v6.py --config configs/qwen_v6_enhanced_temporal.yaml \
        --soft-restart /workspace/outputs/aef_qwen_v5_mixed_scale/epoch_best_epoch161.pt

核心改进 (vs V5):
  - 增强 uniformity: weight 2.5, 有效 batch 48
  - 空间 uniformity: 从 pre_norm_map 采样 256 位置
  - Pixel-level Temporal Cosine Loss: 始终有梯度
  - Pixel-level InfoNCE: Anti-diagonal 空间优化
  - 降低 temporal_magnitude 权重，保留为安全约束
...
def main():
    ...
    if global_rank == 0:
        logger.Print("=" * 70)
        logger.Print(f"  DDP V6 Enhanced Temporal 训练  [GPU 6,7]  —  {cfg.experiment.name}")
        ...
        logger.Print(f"  Temporal cosine pixel: {getattr(cfg.training, 'temporal_cosine_pixel_weight', 0.0):.2f}")
    ...
    # Trainer
    trainer = DDPv6EnhancedTemporalTrainer(cfg, local_rank=local_rank)
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if global_rank == 0:
            logger.Print(
                ...
                f"tc_pixel={losses['tc_pixel']:.4f} ptnce={losses['ptnce']:.4f} "
                f"lr={losses['lr']:.6f}"
            )
