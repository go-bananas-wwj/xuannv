#!/usr/bin/env python3
"""DDP V9.5 Temporal 训练入口 — 8卡 NPU 并行.

用法:
    cd /workspace/xuannv
    torchrun --nproc_per_node=8 \
        scripts/train/train_ddp_v9_5.py --config configs/xuannv_v9_5_temporal.yaml \
        --soft-restart /workspace/outputs/xuannv_backbone_v9_temporal/epoch_best_epoch65.pt \
        --epochs 200
...
from src.training.ddp_v9_5_temporal_trainer import DDPv9_5TemporalTrainer
...
def main():
    ...
    if global_rank == 0:
        logger.Print("=" * 70)
        logger.Print(f"  DDP V9.5 Temporal 训练  [8×NPU]  —  {cfg.experiment.name}")
    ...
    # Trainer
    trainer = DDPv9_5TemporalTrainer(cfg, local_rank=local_rank)
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        if global_rank == 0:
            ...
            pixel_str = f" pixel_change={losses.get('pixel_change', 0.0):.4f}" if losses.get('pixel_change', 0.0) > 0 else ""
            logger.Print(
                ...
                f"{temporal_str}{pixel_str} lr={losses['lr']:.6f}"
            )
