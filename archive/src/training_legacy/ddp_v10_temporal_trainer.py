"""DDP V10 Temporal 训练器 — V9 + Difference Module.

核心改进 (vs V9):
- Bottleneck-Level Difference Module: 显式编码双窗口差分
- change_score: 每个像素的变化概率预测
- change_consistency_loss: 让 change_score 与图像差异对齐
- 保留 V9 所有机制 (temporal loss, teacher-student, uniformity)
"""
...
class DDPv10TemporalTrainer:
    """DDP V10 Temporal 训练器 — Difference Module."""
    ...
    def train_epoch(self, epoch: int, dataloader: DataLoader) -> dict[str, float]:
        ...
        for step, batch in enumerate(dataloader):
            ...
            # 7. ★ V10: Gap-aware Temporal Loss + Difference Module
            ...
            if temporal_enabled and self.temporal_weight_current > 0:
                ...
                if all(k in batch for k in dual_keys):
                    try:
                        with torch.autocast(device_type="npu", dtype=torch.bfloat16, enabled=True):
                            # V10: 使用 encode_dual_window_v10 — 显式差分
    ...
    def save_checkpoint(self, epoch: int, losses: dict) -> None:
        ...
        print(f"[ddp_v10_temporal] Saved checkpoint to {path}")
        ...
        for old_ckpt in ckpts[:-3]:
            old_ckpt.unlink()
            print(f"[ddp_v10_temporal] Removed old checkpoint: {old_ckpt}")
        ...
        for old_ckpt in best_ckpts[:-2]:
            old_ckpt.unlink()
            print(f"[ddp_v10_temporal] Removed old best checkpoint: {old_ckpt}")
