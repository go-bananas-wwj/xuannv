#!/usr/bin/env python3
"""
评估月度 Embedding + ChangeDetectionHead 的 patch-level AUC.
与 benchmark_all_annotated.py 对齐评估协议.
"""
...
from src.models.heads import ChangeDetectionHeadV2, ChangeDetectionHead

EMBEDDING_DIR = Path("/workspace/outputs/aef_qwen_v2/monthly_embeddings_2025")
HEAD_PATH = Path("/workspace/outputs/aef_qwen_v2/monthly_cd_head/monthly_cd_head.pt")
...
def load_head(use_v2: bool = True):
    if use_v2:
        head = ChangeDetectionHeadV2(embedding_dim=128, hidden_dim=128).to(DEVICE)
    else:
...
def main():
    print("=" * 60)
    print("  月度 CD Head Patch-Level Benchmark")
    print("=" * 60)

    head = load_head(use_v2=True)
    ...
    out_path = Path("/workspace/outputs/aef_qwen_v2/monthly_cd_head/benchmark_summary.json")
