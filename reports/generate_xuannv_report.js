...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({
        heading: HeadingLevel.HEADING_1,
        children: [new TextRun("成果二：突破国际现有模型瓶颈，实现时间敏感性与反坍缩双重升级")]
      }),
      new Paragraph({
        style: "BodyText",
        children: [new TextRun("国际主流的地球基础模型存在两大核心瓶颈：一是嵌入向量坍缩，不同地点、不同时相的表征趋于雷同；二是对时间变化不敏感，无法区分不同时间窗口的地表状态，变化检测能力与随机猜测无异。本项目通过独创的反坍缩机制与时序对比学习，使嵌入向量在保持空间判别力的同时，在时间维度上产生显著差异，时间判别力实现量级跃升。变化检测能力从无效提升至可用水平，使模型真正\"看懂\"地球表面的时间演变。在国际上首次在有限数据条件下实现了地球基础模型的时间敏感性突破，为耕地非农化监测、冰川演变追踪、滑坡灾害预警等时序任务提供了全新的技术工具，填补了该领域的国际空白。")]
      }),
