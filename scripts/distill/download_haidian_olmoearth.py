#!/usr/bin/env python3
"""下载海淀区 320 patch 的 OlmoEarth 5 模态输入数据。

复用 download_olmoearth_multimodal.py 核心逻辑，仅修改区域配置。

区域: /workspace/raw/haidian_olmoearth/patches_meta.json 的 320 patch (EPSG:32650, 1280m/128px)
时间: 2025-01-01 ~ 今天
...
输出: /workspace/raw/haidian_olmoearth/<模态>/patch_XXXXXX/<日期或static>.tif

用法:
  conda run -n xuannv python scripts/distill/download_haidian_olmoearth.py --limit 2 --sources s2 s1
  conda run -n xuannv python scripts/distill/download_haidian_olmoearth.py --workers 8
"""
...
OUT_ROOT     = Path("/workspace/raw/haidian_olmoearth")
CRS = "EPSG:32650"
...
def download_modality_for_patch(catalog, src: str, cfg: dict, patch: dict):
    ...
    for day, it in selected:
        ...
        if dst.exists():
            ...
            continue
        try:
            arr = load_with_retry(it, cfg["assets"], geobox)
            if landsat_map:
