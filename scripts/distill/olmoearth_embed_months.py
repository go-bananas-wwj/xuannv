...
TARGET_MONTHS = [4, 6, 8, 9, 10]   # 对应 AUC 评估时间段


def pick_month(modality, patch, target_month):
    """选最接近 target_month 的一景（偏好当月，退而取相邻）。"""
...
def main():
    ...
    for month in TARGET_MONTHS:
        ...
        # 6月已有，跳过
        if os.path.exists(cache_f) and month == 6:
        ...
        if os.path.exists(cache_f):
            ...
            for i, pid in enumerate(patches):
                if str(i) in c.files or pid in c.files:
                    embs[pid] = c.get(pid, c.get(str(i)))
