#!/usr/bin/env python3
"""
对AEF官方嵌入做KNN下游评估（哈尔滨新区：WorldCover / JRC Water / Dynamic World）。
"""
...
def evaluate_knn(embeddings, labels, mapping=None, k=5, sample_pixels=10000):
    all_emb, all_label = [], []
...
def main():
    embedding_path = "/workspace/outputs/aef_official_embeddings/aef_harbin_2025.npz"
    data_root = Path("/workspace/raw/phase1_harbin/harbin_scenes_cloud_filtered")
    output_path = "/workspace/outputs/aef_official_embeddings/knn_aef_harbin.json"
    k = 5
    sample_pixels = 10000
    ...
    if labels:
        ...
        result = evaluate_knn(embeddings, labels, mapping=mapping, k=k, sample_pixels=sample_pixels)
        print(f"  Acc: {result['accuracy']:.4f}, mIoU: {result['miou']:.4f}")
        print(f"  Classes: {result['classes']}")
        results["worldcover"] = result
    else:
        print("  No labels found")
        results["worldcover"] = None
    ...
    print(f"  Labels found: {len(labels)}/{len(embeddings)}")
    if labels:
        result = evaluate_knn(embeddings, labels, mapping=None, k=k, sample_pixels=sample_pixels)
        print(f"  Acc: {result['accuracy']:.4f}, mIoU: {result['miou']:.4f}")
        print(f"  Classes: {result['classes']}")
        results["jrc_water"] = result
    else:
        print("  No labels found")
        results["jrc_water"] = None
