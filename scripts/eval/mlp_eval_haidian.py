...
推荐用法（先用 extract_embeddings.py 提取，再用 NPZ 评估，最快）：
    ...
    python scripts/eval/extract_embeddings.py --config configs/config_haidian_v25.yaml \\
        --checkpoint epoch_best_epoch78.pt --output-dir evaluation/results/eval_v25_0531/ --format npz

    # 2. 再评估（CPU 即可，秒级完成）
    python scripts/eval/mlp_eval_haidian.py \\
        --embedding-file out/eval_v25_0531/patch_embeddings_shard0.npz \\
        --output-dir evaluation/results/eval_v25_0531/mlp/
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="evaluation/results/eval_v25_0531/mlp/")
