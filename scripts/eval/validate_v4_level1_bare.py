#!/usr/bin/env python3
"""V4 Level 1: Backbone bare AUC on 20 annotated patches."""
...
CONFIG_PATH = "/workspace/xuannv/configs/qwen_v4_official.yaml"
CKPT_V4 = "/workspace/outputs/aef_qwen_v4_official/epoch_best_epoch231.pt"
ANNOT_DIR = "/workspace/哈尔滨松北新区变化检测汇总文件/变化检测shp文件"
GRID_PATH = "/workspace/index/harbin/grid/harbin_grid.geojson"
DEVICE = "cuda:0"  # GPU 7 (CUDA_VISIBLE_DEVICES=7)
...
print("="*60)
print("  V4 Level 1: Backbone Bare AUC (20 annotated patches)")
print(f"  Checkpoint: {CKPT_V4}")
print("="*60)
...
print("\n加载 V4 模型...")
model_v4, dataset_v4 = load_model(CKPT_V4)
...
def extract_emb(model, dataset, patch_idx, valid_start_ms, valid_end_ms):
    ...
    # V4: model.eval() already applies L2 norm via VMFBottleneck
    # Double normalize to be safe
...
def compute_change_map(emb_before, emb_after):
    ...
    # Already normalized by model, but ensure
...
for i, (pid, changes) in enumerate(test_patches):
    if pid not in dataset_v4.patches:
        continue
    
    pidx = dataset_v4.patches.index(pid)
    print(f"  [{i+1}/{len(test_patches)}] {pid}... ", end="", flush=True)
    t0 = time.time()
    
    try:
        eb_v4 = extract_emb(model_v4, dataset_v4, pidx, BEFORE_WINDOW[0], BEFORE_WINDOW[1])
        ea_v4 = extract_emb(model_v4, dataset_v4, pidx, AFTER_WINDOW[0], AFTER_WINDOW[1])
        cd_v4 = compute_change_map(eb_v4, ea_v4)
        ...
        dist_mean = float(cd_v4.mean())
        dist_changed = float(cd_v4[change_mask > 0].mean()) if n_changed > 0 else 0.0
        dist_unchanged = float(cd_v4[change_mask == 0].mean())
        
        print(f"AUC={auc_v4:.3f}, dist_mean={dist_mean:.4f}, "
...
# ──────────────────────────────────────────
print("\n" + "="*60)
print("  V4 Level 1 结果汇总")
print("="*60)

if results_v4:
    print(f"\nV4 Backbone Bare: {len(results_v4)} patches")
    print(f"  AUC mean  = {np.mean(results_v4):.3f}")
    print(f"  AUC median= {np.median(results_v4):.3f}")
    print(f"  AUC std   = {np.std(results_v4):.3f}")
    print(f"  AUC min   = {np.min(results_v4):.3f}")
    print(f"  AUC max   = {np.max(results_v4):.3f}")
    print(f"  AUC > 0.5: {sum(1 for a in results_v4 if a > 0.5)}/{len(results_v4)}")
    ...
    delta = np.mean(results_v4) - 0.49
    print(f"  V4 → V2 变化: {np.mean(results_v4):.3f} vs 0.49 ({'↑+' if delta > 0 else '↓'}{abs(delta):.3f})")

# 保存详细结果
import os
os.makedirs("/workspace/outputs/aef_qwen_v4_official/eval", exist_ok=True)
out_path = "/workspace/outputs/aef_qwen_v4_official/eval/level1_bare_auc.json"
import json as _json
with open(out_path, "w") as f:
    _json.dump({
        "model": "V4_official_epoch231",
        "n_patches": len(results_v4),
        "auc_mean": float(np.mean(results_v4)) if results_v4 else None,
        "auc_median": float(np.median(results_v4)) if results_v4 else None,
        "auc_std": float(np.std(results_v4)) if results_v4 else None,
        "auc_min": float(np.min(results_v4)) if results_v4 else None,
        "auc_max": float(np.max(results_v4)) if results_v4 else None,
