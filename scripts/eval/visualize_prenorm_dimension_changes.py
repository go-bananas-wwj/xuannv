#!/usr/bin/env python3
"""
可视化 V5 Pre-Norm Embedding（无 L2 Normalization）每个维度的变化强度.

与 L2 norm 版本对比，检查变化信息是否在 pre-norm 空间更强.
"""
...
def extract_prenorm_emb(model, dataset, patch_idx, valid_start_ms, valid_end_ms):
    ...
    # pre_norm_map: [1, D, H, W] in pre-norm space
...
start_ms, end_ms = MONTH_TO_MS[PERIOD_MONTHS[0]]
emb_before = extract_prenorm_emb(model, dataset, pidx, start_ms, end_ms)
start_ms, end_ms = MONTH_TO_MS[PERIOD_MONTHS[1]]
...
print(f"\nL2-Norm Top 10:")
for s in stats_l2[:10]:
    sign = "+" if s["changed_mean"] > s["unchanged_mean"] else "-"
    print(f"  Dim {s['dim']:3d}: chg={s['changed_mean']:+.5f}, unchg={s['unchanged_mean']:+.5f}, diff={s['diff']:.5f} [{sign}]")

# ── 6. 可视化 Pre-Norm Top 维度 ──
N_TOP = 16
...
plt.suptitle(f"{PATCH_ID}  Pre-Norm Embedding: Top {N_TOP} Dimensions by Change Sensitivity\n"
             ...
             f"Period: {PERIOD_MONTHS[0]} → {PERIOD_MONTHS[1]}",
             fontsize=14, fontweight="bold")
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
fig.savefig(OUT_DIR / f"prenorm_dim_change_heatmap_{PATCH_ID}.png", dpi=150, bbox_inches="tight")
plt.close(fig)
print(f"\n  Saved: {OUT_DIR / f'prenorm_dim_change_heatmap_{PATCH_ID}.png'}")

# ── 7. 对比箱线图: Pre-Norm vs L2-Norm ──
fig, axes = plt.subplots(1, 2, figsize=(20, 6))

def plot_box(ax, stats_list, title, color_changed="#e74c3c", color_unchanged="#3498db"):
    changed_vals = []
    unchanged_vals = []
    dim_labels = []
    for s in stats_list[:40]:
        d = s["dim"]
        changed_vals.append(delta[d][change_mask > 0].flatten() if title.startswith("Pre") else delta_l2[d][change_mask > 0].flatten())
        unchanged_vals.append(delta[d][change_mask == 0].flatten() if title.startswith("Pre") else delta_l2[d][change_mask == 0].flatten())
...
report = {
    "patch_id": PATCH_ID,
    "period": f"{PERIOD_MONTHS[0]}~{PERIOD_MONTHS[1]}",
    "pre_norm": {
