#!/usr/bin/env python3
"""预提取 V5 Mixed Scale 模型的 PRE-NORM embedding (不 L2 归一化).

与 extract_v5_monthly_embeddings.py 的区别:
  normalize=False → 保留 pre-norm embedding 的幅度信息
"""
...
OUTPUT_DIR = Path("/workspace/outputs/aef_qwen_v5_mixed_scale/monthly_embeddings_2025_prenorm")
...
def main():
    ...
    meta_records = []
    for pid in tqdm(all_patches, desc="Extracting pre-norm embeddings"):
        ...
        for month, (ws, we) in MONTHLY_WINDOWS.items():
            ...
            if out_path.exists():
                continue
            try:
                emb = extract_embedding_map(model, dataset, pidx, ws, we, device, normalize=False)
    ...
    print(f"\nDone. Saved {len(meta_records)} pre-norm embeddings to {OUTPUT_DIR}")
