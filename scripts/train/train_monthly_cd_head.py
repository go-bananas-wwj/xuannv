#!/usr/bin/env python3
"""
冻结 V2 backbone，用 2025 年单月份 embedding 对训练增强版 ChangeDetectionHead.

支持:
  - ChangeDetectionHeadV2 / V3 (binary)
  - MultiClassChangeDetectionHead (3-class)
  - Embedding 空间数据增强 (flip, rotate, noise, channel dropout)
...
用法:
    cd /workspace/xuannv
    CUDA_VISIBLE_DEVICES=6 python scripts/train/train_monthly_cd_head.py --head_type v3
"""
...
class EmbeddingAugment:
    ...
    对 emb_before / emb_after / mask 做同步空间变换，
    并给 embedding 加噪声和通道 dropout。
    """
...
def main():
    ...
    print("=" * 70)
    print(f"  月度 Embedding Change Detection Head 训练  [{args.head_type}]")
    print("=" * 70)
    ...
    if not records:
        print("❌ No valid records found. Run extract_monthly_embeddings_2025.py first.")
        return
