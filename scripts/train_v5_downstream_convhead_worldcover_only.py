#!/usr/bin/env python3
"""仅运行 WorldCover 的 PixelConvHead 训练（修复 class_remap 后补跑）."""
...
def train_pixel_conv_head(task_name, ds, n_classes, is_binary=False, epochs=40, batch_size=4, lr=1e-3):
    ...
    save_path = OUT_DIR / "worldcover_convhead.pt"
    torch.save({
...
def main():
    print("=" * 60)
    print("WorldCover PixelConvHead 补跑（修复 class_remap）")
    print("=" * 60)
    ...
    result = train_pixel_conv_head("WorldCover", ds_wc, n_classes=wc_n, is_binary=False, epochs=40)
    ...
    # Remove existing WorldCover if present
    existing = [r for r in existing if r["task"] != "WorldCover"]
