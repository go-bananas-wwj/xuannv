"""DDP V6.5 Gap-Aware Temporal 训练器 — 修复 V6 过强时序损失.

核心改进 (vs V6):
- gap_aware_temporal_cosine_loss: 根据时间 gap 设定 target cos_sim
  gap=0 → target=1 (相似), gap=max → target=-1 (相反)
- 降低 temporal 权重, 增强 uniformity/variance
- 保留空间 uniformity 和 pixel InfoNCE
"""
...
def _gather_spatial_embeddings(pre_norm_map: torch.Tensor, num_samples: int) -> torch.Tensor:
    ...
    if total_spatial >= num_samples:
        flat = pre_norm_map.permute(0, 2, 3, 1).reshape(-1, D)
...
class DDPv6_5GapAwareTrainer:
    """DDP V6.5 Gap-Aware 训练器 — GPU 6/7 双卡."""
    ...
    def train_epoch(self, epoch: int, dataloader: DataLoader) -> dict[str, float]:
        ...
        for step, batch in enumerate(dataloader):
            ...
            if dist.is_initialized() and self.world_size > 1:
                ...
                pre_norm_map = student_out.pre_norm_map
            ...
            time_gap_ms = torch.abs(center_w2 - center_w1).float()
            ...
            # 9. Gap-Aware Temporal Cosine Loss (★ V6.5 核心)
            gap_aware_w = getattr(t, 'temporal_cosine_pixel_weight', 0.0)
            gap_aware = torch.tensor(0.0, device=self.device)
            if gap_aware_w > 0:
                gap_aware = gap_aware_temporal_cosine_loss(
                    pre_w1.float(), pre_w2.float(),
                    time_gap_ms=time_gap_ms,
                    max_gap_ms=getattr(t, 'temporal_max_gap_ms', six_month_ms),
            ...
            if torch.isnan(total) or torch.isinf(total):
                if self.global_rank == 0 and step % 10 == 0:
                    print(f"  [WARNING] NaN/Inf total at step {step}, skipping. "
                          ...
                          f"gap_aware={gap_aware.item():.3f} ptnce={ptnce.item():.3f}")
            ...
            if self.global_rank == 0 and step % 20 == 0:
                print(f"  [V6.5] frame_drop={perturb_stats['frame_drop_ratio']:.2f} "
    ...
    def save_checkpoint(self, epoch: int, losses: dict) -> None:
        ...
        print(f"[ddp_v6_5] Saved checkpoint to {path}")
        ...
        for old_ckpt in ckpts[:-3]:
            old_ckpt.unlink()
            print(f"[ddp_v6_5] Removed old checkpoint: {old_ckpt}")
        ...
        for old_ckpt in best_ckpts[:-2]:
            old_ckpt.unlink()
            print(f"[ddp_v6_5] Removed old best checkpoint: {old_ckpt}")
