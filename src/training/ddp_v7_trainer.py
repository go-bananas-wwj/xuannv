            ...
            with torch.autocast(device_type="npu", dtype=torch.float16, enabled=True):
                ...
                if temporal_w > 0 and "valid_start_w1" in batch:
                    try:
                        ...
                        # 使用 pixel-level cosine loss（Round 9: 支持空间加权）
                        use_spatial_weight = getattr(t, 'temporal_use_spatial_weight', False)
                        temporal = temporal_cosine_pixel_loss(
                            emb_w1, emb_w2,
                            temperature=getattr(t, 'temporal_magnitude_temperature', 0.1),
